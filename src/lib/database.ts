...
class DatabaseManager {
  ...
  private initializeDatabase() {
    ...
    try {
      this.db.exec(`ALTER TABLE configurations ADD COLUMN title TEXT`);
    } catch (error) {
      // Column already exists, ignore error
    }

    try {
      this.db.exec(`ALTER TABLE configurations ADD COLUMN description TEXT`);
    } catch (error) {
      // Column already exists, ignore error
    }
  ...
  async saveConfiguration(config: Omit<ConfigurationSubmission, 'createdAt' | 'updatedAt'>): Promise<void> {
    ...
    const stmt = this.db.prepare(`
      INSERT OR REPLACE INTO configurations (
        id, title, description, industry_focus, other_industry, core_platforms, operating_system,
    ...
    stmt.run(
      ...
      config.contactInfo,
      now, // created_at will be set to now for new records or kept for existing ones
      now,
      config.status
    );
  }
