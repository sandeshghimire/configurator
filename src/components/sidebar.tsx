...
import { StaggeredList, HoverScale, ScaleIn } from '@/components/animated';
        ...
        <nav className="space-y-1">
          <StaggeredList staggerDelay={0.05}>
            {menuItems.map((item, index) => {
              ...
              return (
                <HoverScale key={item.href} scale={1.02} className="block">
                  <Button
                    variant="ghost"
                    className={cn(
                  ...
                  >
                    <Link href={item.href} className="flex items-center space-x-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <span className={cn(
                          "text-xs font-medium block truncate transition-colors duration-200",
